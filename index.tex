...
\title{\textit{In situ} tuning of gold nanorod plasmon through oxidative cyanide
etching}

\keywords{Gold nanorods, Plasmon, Etching, Oxidation, Cyanide}
...
of the particle and can be found between $550\nm$ for spheres with AR of $1$ to
...
requires precise and \em{in situ} control over the nanoparticles' plasmon
...
resonance. Furthermore, these methods do not provide any \em{in situ} adjustment
of the SPR, any change of which requires a new synthesis.
...
studies\cite{Carbo-Argibay2007,Rodriguez-Fernandez2005,Jana2002}. In those
cases, the capping agent will induce different reactivities on the sides than on
...
In this work we present a new approach for precise and \em{in situ} tuning of
plasmon resonances of single gold nanorods isolated and immobilized on a glass
...
Supplementary Material for SEM images and bulk extinction spectra of nanorods
as synthesized).

Single-particle measurements were done on a home-built confocal microscope
(Figure \ref{fig:setup}a). A $532\nm$ laser was used for exciting the
particles. The excitation power was $300\uW$ at the back aperture of the
objective (Olympus $60\times$, $\textrm{NA}\,0.9$ air). The luminescence signal
was then filtered with two $532\nm$ notch filters and was detected by either an
avalanche photodiode or a liquid-nitrogen-cooled CCD-spectrometer (Acton
...
(cetyltrimethylammonium bromide, CTAB.) To perform the measurements, the
samples were mounted on a flowcell and the initial spectra were taken with the
rods immersed in Milli-Q water. Figure \ref{fig:setup}c shows an example of the
luminescence spectrum of a single particle. Having this initial characterization
allowed us to discard clusters of rods\cite{Funston2009} from the study. Next a
solution of KCN was flowed with concentrations ranging from $3\uM$ to $80\uM$.
In each case the spectra of approximately $10$ different particles were acquired
consecutively after focusing on each one. The time resolution varies according
to the exposure time and number of particles; in this work a spectrum of each
particle was taken at least every minute.
...
prominent for the less intense curves. Water exhibits a broad Raman emission
band between $3000\,\text{cm}^{-1}$ and $3600\,\text{cm}^{-1}$. When excited at
$532\nm$ it results in emission between roughly $630\nm$ and $650\nm$ and can be
observed directly from the background spectra as shown in Figure S3. The fact
that the background couldn't be subtracted from the spectra of the particles
indicates a nonlinear effect. We therefore attribute this emission to enhanced
Raman scattering from water\cite{Snow1985}. 
...
$1\nm/\textrm{min}$. This value however largely depends on the initial particle
size used in the simulation. The relative volume therefore can be used as a
scale invariant useful for comparing both simulations and experimental data, as
is shown in the second inset of Figure \ref{fig:simulations}a.

The luminescence intensity of gold nanorods is proportional to their volume. It
...
\begin{figure}[p]
 ...
 with Lorentzians. The left inset show the maximum scattering cross section as
...
rate is defined as the plasmon shift given by etching a thickness of $o.5\nm$
...
slight increase in the aspect ratio but this is obscured by the broad
...
\begin{suppinfo}
Bulk extinction spectra, SEM images and background spectrum are available.
